...
async def send_email_msg(
    ...
    smtp_port: int = 465,
    use_ssl: bool = True  # QQ邮箱需要SSL
) -> bool:
    """
    发送邮件消息（支持QQ邮箱）
    
    注意：QQ邮箱使用SSL连接，端口465
    """
    try:
        # 创建邮件对象
        ...
        email_msg['To'] = ', '.join(receiver_emails)
...
def send_sync(
    ...
    """同步发送邮件"""
    if use_ssl:
        # 使用SMTP_SSL连接（适用于QQ邮箱）
        with smtplib.SMTP_SSL(smtp_server, smtp_port, context=ssl.create_default_context()) as server:
            server.login(sender_email, sender_password)
            server.sendmail(sender_email, receiver_emails, email_msg.as_string())
    else:
        # 使用普通SMTP连接（适用于Gmail等）
